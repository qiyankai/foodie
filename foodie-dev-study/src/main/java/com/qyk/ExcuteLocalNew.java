...
public class ExcuteLocalNew {
    ...
    public static void main(String[] args) {
        ...
        ExecutorService pool = new ThreadPoolExecutor(50, 300,
                0L, TimeUnit.MILLISECONDS,
                new LinkedBlockingQueue<Runnable>(1024), namedThreadFactory, new ThreadPoolExecutor.AbortPolicy());

        for (int i = 1; i <= threadSize; i++) {
            pool.execute(new Thread(new RefreshBlogThreadNew(i * 10000, localBlogUrl, localBlogUrlCount, count, proxyOrderId), "thread-refresh-" + i));
        }
    }
